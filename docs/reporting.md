...
JSON output. The default registry contains [ML001](ml001.md) and [ML002](ml002.md).
...
tests; the installed CLI uses a fresh default registry containing ML001 and
ML002. An explicit registry is used exactly as supplied. `--disable-rule` can
disable either built-in rule independently; unknown rule IDs are invocation errors. It never loads rules
